...
static void		rotate(t_param *param, int right)
{
	...
	param->dir_x =
		param->dir_x * cos(rot_speed) - param->dir_y * sin(rot_speed);
	param->dir_y = old_dir_x * sin(rot_speed) + param->dir_y * cos(rot_speed);
	old_plane_x = param->plane_x;
	param->plane_x =
		param->plane_x * cos(rot_speed) - param->plane_y * sin(rot_speed);
	param->plane_y =
		old_plane_x * sin(rot_speed) + param->plane_y * cos(rot_speed);
}

static int		key_down(t_param *param, SDL_Event *event)
{
...
int				catch_event(t_param *param, t_sdl *sdl, SDL_Event *event)
{
	if (SDL_PollEvent(event))
	{
		...
		else if (event->type == SDL_KEYDOWN && key_down(param, event))
			return (1);
